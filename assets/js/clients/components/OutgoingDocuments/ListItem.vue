<template>
  ...
  <tbody>
    <tr  >
		<td class="col-md-1"   @click="navigateToAppeal">
			<span v-if="appealLevel" >{{ value.case_id }}</span>
   ...
   <td class="col-md-1"   @click="navigateToAppeal">{{ patientName }}</td>

    <td class="col-md-2"   @click="navigateToAppeal">
		<div v-if="!hideAgency">
...
export default {
	...
	methods: {
		async refresh() {
			try {
				...
				console.log("all request ", this.entity);
			} catch (e) {
				this.error = true;
    ...
    params: { id: this.value.case_id, case_request_id: this.entity.id },
  });
}
},
