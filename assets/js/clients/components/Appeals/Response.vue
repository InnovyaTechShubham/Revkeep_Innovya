	...
	<p class="mb-0">
		...
		{{ outgoingProfile.electronic_website }}
					...
					<!-- <b-form-group label="Delivery Method" label-cols-lg="4">
						<b-form-select label=" Delivery Method " v-model="selectedOptionMethod" class="mt-2"
					...
					<div v-if="selectedSubmitTo === 'facility' || selectedSubmitTo === 'agency'">
						<b-form-group label="Delivery Method" label-cols-lg="4">
						<b-form-select label = "deliveryMethod" v-model="selectedDeliveryMethod" class="mt-2" @change="handleSubmitToChange">
							<option value="fax">Fax</option>
							<option value="email">Email</option>
					...
					<div v-if="selectedDeliveryMethod === 'manual'">
						<!-- Render fields for User -->
						<label for="carrier">Carrier:</label>
						<input type="text" v-model="carrier" />
...
label {
  ...
  display: block; /* Make labels block-level for proper alignment */
}
